...
async function getCompliance() {
  try {
    ...
    const r = await fetch(`${base}/api/insightcore/studio/governance/compliance`, {
      cache: 'no-store',
    });
    if (!r.ok) return null;
...
export default async function GovernanceCompliancePage() {
  const item = await getCompliance();

  return (
    <main className="min-h-screen bg-slate-950 px-6 py-10 text-white">
      <div className="mx-auto max-w-6xl">
        <h1 className="text-3xl font-semibold tracking-tight">Governance compliance</h1>
