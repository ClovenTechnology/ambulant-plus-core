...
async function getUncertainty() {
  try {
    ...
    const r = await fetch(`${base}/api/insightcore/studio/uncertainty`, {
      cache: 'no-store',
    });
    if (!r.ok) return null;
...
export default async function UncertaintyPage() {
  const item = await getUncertainty();

  return (
    <main className="min-h-screen bg-slate-950 px-6 py-10 text-white">
      <div className="mx-auto max-w-6xl">
        <div className="text-sm uppercase tracking-[0.24em] text-cyan-300/80">InsightCore Studio</div>
        <h1 className="mt-3 text-3xl font-semibold tracking-tight">Uncertainty</h1>
        <p className="mt-3 max-w-3xl text-slate-300">
          Review measurement, inference and clinical uncertainty outputs.
        </p>
        ...
        <div className="mt-8 rounded-[24px] border border-white/10 bg-white/5 p-5">
          {item ? (
            ...
            <div className="text-slate-300">No uncertainty record yet.</div>
          )}
        </div>
      </div>
    </main>
