...
export default async function FhirPage() {
  const item = await getFhir();

  return (
    <main className="min-h-screen bg-slate-950 px-6 py-10 text-white">
      <div className="mx-auto max-w-6xl">
        <div className="text-sm uppercase tracking-[0.24em] text-cyan-300/80">InsightCore Studio</div>
        <h1 className="mt-3 text-3xl font-semibold tracking-tight">FHIR projections</h1>
        <p className="mt-3 max-w-3xl text-slate-300">
          Inspect FHIR-facing episode intelligence outputs.
        </p>
        ...
        <div className="mt-8 rounded-[24px] border border-white/10 bg-white/5 p-5">
          {item ? (
            ...
            <div className="text-slate-300">No FHIR projection yet.</div>
          )}
        </div>
      </div>
    </main>
