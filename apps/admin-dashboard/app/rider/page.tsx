  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!mounted) return;
        setRows([]);
        setErr(e?.message || 'Unable to load rider jobs.');
      } finally {
  ...
  return (
    <main className="p-4 md:p-6 max-w-4xl mx-auto space-y-4">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold">Rider Jobs (CarePort)</h1>
          <p className="text-xs text-gray-500 mt-1">
            Mobile-friendly list of active CarePort jobs for riders.
          </p>
        </div>
        <div className="flex flex-col items-end gap-1">
          <div className="flex items-center gap-2 text-xs">
            <Link href="/careport" className="underline text-indigo-700">
              CarePort dashboard
            </Link>
            <Link href="/careport/orders" className="underline text-gray-600">
              Rider timelines
            </Link>
          </div>
      ...
      <section className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <input
          ...
          placeholder="Search by patient, job ID, rider…"
          value={search}
              ...
              >
                ...
                <div className="flex justify-between items-center gap-3">
                  <div className="flex-1 text-[11px] text-gray-600">
                    <div className="font-medium mb-0.5">Rider</div>
                    <div>
                      {job.rider?.name || '—'}{' '}
                    ...
                    <div className="text-gray-500">
                      {job.rider?.vehicle || '—'}{' '}
                      {job.rider?.plate ? `• ${job.rider.plate}` : ''}
                    </div>
                  </div>
