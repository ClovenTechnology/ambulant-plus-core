    ...
    <main className="space-y-6 p-4 lg:p-6">
      <header className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {canScheduleInterview ? <a href="#interview-scheduling" className="inline-flex items-center gap-2 rounded-xl bg-cyan-700 px-4 py-2 text-sm font-semibold text-white"><CalendarClock className="h-4 w-4" />Schedule interview</a> : null}
          {actions.map((action) => (
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-6">
          ...
          <section id="interview-scheduling" className="scroll-mt-24 space-y-4 rounded-3xl border bg-white p-5 shadow-sm">
            ...
            {!interviewPermissions.canRead ? <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">Interview details are not available for your current role.</div> : null}

            {interviewPermissions.canRead && !interview && !canScheduleInterview ? <div className="rounded-xl border bg-slate-50 p-4 text-sm text-slate-600">{application?.status === 'SHORTLISTED' ? 'Interview scheduling is not available for your current role.' : 'Move the application to Shortlisted when the review is complete. Interview scheduling will then become available here.'}</div> : null}
