...
function Action({ icon: Icon, children }: { icon: any; children: React.ReactNode }) {
  return <button type="button" disabled title="Enabled when the communication foundation is installed" className="inline-flex items-center gap-2 rounded-xl border bg-slate-50 px-3 py-2 text-sm text-slate-400 opacity-70"><Icon className="h-4 w-4" />{children}</button>;
}
...
export default function AdminStaffProfilePage({ params }: { params: { id: string } }) {
  ...
  return <main className="space-y-6 p-4 lg:p-6">
    ...
    {item && <>
      <section className="rounded-3xl border bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="text-xs font-semibold uppercase tracking-[0.18em] text-teal-700">{item.lifecycleState}</div><div className="mt-2 text-lg font-semibold">{item.department?.name || 'No department'} · {item.designation?.name || 'No designation'}</div></div><div className="flex flex-wrap gap-2"><Action icon={MessageSquare}>Message</Action><Action icon={Phone}>Audio call</Action><Action icon={Video}>Video call</Action><Action icon={CalendarPlus}>Schedule meeting</Action></div></div><p className="mt-3 text-xs text-slate-500">Communication controls are intentionally disabled until the shared communication/meeting engine is installed in the next coordinated phase.</p></section>
