...
function Action({ icon: Icon, children, href, title }: { icon: any; children: React.ReactNode; href?: string; title?: string }) {
  ...
  return <button type="button" disabled title={title || "Action not available for your current permission set"} className="inline-flex items-center gap-2 rounded-xl border bg-slate-50 px-3 py-2 text-sm text-slate-400 opacity-70"><Icon className="h-4 w-4" />{children}</button>;
}
...
export default function AdminStaffProfilePage({ params }: { params: { id: string } }) {
  ...
  return <main className="space-y-6 p-4 lg:p-6">
    ...
    {item && <>
      <section className="rounded-3xl border bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="text-xs font-semibold uppercase tracking-[0.18em] text-teal-700">{item.lifecycleState}</div><div className="mt-2 text-lg font-semibold">{item.department?.name || 'No department'} · {item.designation?.name || 'No designation'}</div></div><div className="flex flex-wrap gap-2"><Action icon={MessageSquare} href={!pending && data?.permissions?.canUseCommunications ? `/admin/communications?staffId=${encodeURIComponent(item.id)}` : undefined} title={data?.permissions?.canUseCommunications ? 'Open persistent staff conversation' : 'communications.use permission required'}>Message</Action><Action icon={Phone} href={!pending && data?.permissions?.canUseCommunications ? `/admin/communications?staffId=${encodeURIComponent(item.id)}&call=audio` : undefined} title={data?.permissions?.canUseCommunications ? 'Start a canonical direct audio call' : 'communications.use permission required'}>Audio call</Action><Action icon={Video} href={!pending && data?.permissions?.canUseCommunications ? `/admin/communications?staffId=${encodeURIComponent(item.id)}&call=video` : undefined} title={data?.permissions?.canUseCommunications ? 'Start a canonical direct video call' : 'communications.use permission required'}>Video call</Action><Action icon={CalendarPlus} href={!pending && data?.permissions?.canCreateMeetings ? `/admin/meetings/new?staffId=${encodeURIComponent(item.id)}` : undefined} title={data?.permissions?.canCreateMeetings ? 'Schedule a meeting with this staff member' : 'meetings.create permission required'}>Schedule meeting</Action></div></div><p className="mt-3 text-xs text-slate-500">Messaging is persistent. Audio/video calls create canonical <code>DIRECT_CALL</code> Meeting records and reuse the shared RTC authority.</p></section>
