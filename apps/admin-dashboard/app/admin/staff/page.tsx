...
function Pill({ value }: { value: string }) {
  return <span className={`inline-flex rounded-full border px-2 py-0.5 text-[11px] font-medium ${tone(value)}`}>{value.replaceAll('_', ' ')}</span>;
}
    ...
    <main className="space-y-6 p-4 lg:p-6">
      <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm text-slate-600">Canonical internal identity directory for approved Admin staff and pending applicants. Organisational identity, roles, presence and lifecycle are server-authorised.</p>
        </div>
      ...
      <section className="rounded-3xl border bg-white p-4 shadow-sm">
        <form className="grid gap-3 xl:grid-cols-[minmax(280px,2fr)_repeat(5,minmax(140px,1fr))_auto]" onSubmit={(event) => { event.preventDefault(); setPage(1); load(); }}>
          ...
          <select value={state} onChange={(e) => { setPage(1); setState(e.target.value); }} className="rounded-xl border px-3 py-2.5 text-sm"><option value="">All states</option>{['ACTIVE','PENDING','LEAVE','SUSPENDED','ARCHIVED'].map((x) => <option key={x}>{x}</option>)}</select>
          ...
          <select value={presence} onChange={(e) => { setPage(1); setPresence(e.target.value); }} className="rounded-xl border px-3 py-2.5 text-sm"><option value="">All presence</option>{['AVAILABLE','BUSY','IN_MEETING','DO_NOT_DISTURB','OFFLINE'].map((x) => <option key={x}>{x}</option>)}</select>
      ...
      <section className="overflow-hidden rounded-3xl border bg-white shadow-sm">
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead className="border-b bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3">Staff</th><th className="px-4 py-3">Organisation</th><th className="px-4 py-3">Roles</th><th className="px-4 py-3">State</th><th className="px-4 py-3">Presence</th><th className="px-4 py-3"></th></tr></thead>
            <tbody className="divide-y">
              ...
              {!busy && !(data?.items || []).length && <tr><td colSpan={6} className="px-4 py-12 text-center text-sm text-slate-500"><UsersRound className="mx-auto mb-3 h-7 w-7" />No staff match the current filters.</td></tr>}
            </tbody>
          </table>
        </div>
