    ...
    <main className="space-y-6 p-4 lg:p-6">
      <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm text-slate-500">Reusable recruitment defaults across opportunities, application forms, interview evaluation forms and canonical Staff onboarding.</p>
        </div>
      ...
      <section className="grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
          ...
          <div className="mt-4 space-y-3">
            {(payload?.templates || []).map((item: any) => (
              <button key={item.id} type="button" onClick={() => editTemplate(item)} className="w-full rounded-2xl border p-4 text-left hover:bg-slate-50">
                <div className="flex items-center justify-between gap-3"><div className="font-semibold">{item.name}</div><span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-semibold">{item.status}</span></div>
                <div className="mt-1 text-xs text-slate-500">{item.key} · {item.opportunityType || 'Any opportunity type'}</div>
        ...
        <div className="rounded-3xl border bg-white p-5 shadow-sm">
          ...
          <label className="mt-4 block text-sm"><span className="mb-1 block font-medium">Applicant onboarding message</span><textarea value={settings.onboardingMessage} onChange={(event) => setSettings((current: any) => ({ ...current, onboardingMessage: event.target.value }))} className="min-h-28 w-full rounded-xl border p-3" placeholder="Message sent when canonical Staff onboarding begins" /></label>
          <label className="mt-4 flex items-start gap-3 text-sm"><input type="checkbox" checked={settings.requireCredentialBeforeApproval} onChange={(event) => setSettings((current: any) => ({ ...current, requireCredentialBeforeApproval: event.target.checked }))} className="mt-1" /><span><span className="font-medium">Require Admin credential before final Staff activation</span><span className="mt-1 block text-xs text-slate-500">The existing role-request approval gate remains authoritative.</span></span></label>
      ...
      <form onSubmit={saveTemplate} className="rounded-3xl border bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          <label className="text-sm"><span className="mb-1 block font-medium">Opportunity type</span><select value={draft.opportunityType} onChange={(event) => setDraft((current: any) => ({ ...current, opportunityType: event.target.value }))} className="w-full rounded-xl border px-3 py-2">{OPPORTUNITY_TYPES.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
          ...
          <label className="text-sm"><span className="mb-1 block font-medium">Status</span><select value={draft.status} onChange={(event) => setDraft((current: any) => ({ ...current, status: event.target.value }))} className="w-full rounded-xl border px-3 py-2"><option>ACTIVE</option><option>INACTIVE</option><option>ARCHIVED</option></select></label>
