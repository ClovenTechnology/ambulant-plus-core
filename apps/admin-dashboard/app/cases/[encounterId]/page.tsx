...
export default function CaseOverview({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const r = await fetch(
          `${CLIN}/api/summary/${encodeURIComponent(encounterId)}`,
          { cache: 'no-store' },
        );
  ...
  const cancelAppointment = async (id: string) => {
    ...
    await fetch(`${CLIN}/api/appointments/${id}/cancel`, {
      method: 'PUT',
    });
  ...
  const rescheduleAppointment = async (id: string) => {
    ...
    const startsAt = prompt(
      'New start time (ISO, e.g. 2025-09-08T10:00:00Z):',
    );
        ...
        <>
          <section className="bg-white border rounded p-4">
            ...
            <div className="text-sm grid md:grid-cols-2 gap-2">
              <div>
                <span className="text-gray-500">ID:</span> {sum.encounter.id}
              </div>
              <div>
                <span className="text-gray-500">Case:</span>{' '}
                {sum.encounter.caseId}
              </div>
              <div>
                <span className="text-gray-500">Patient:</span>{' '}
                {sum.encounter.patientId}
              </div>
              <div>
                <span className="text-gray-500">Clinician:</span>{' '}
                {sum.encounter.clinicianId || '-'}
              </div>
              <div>
                <span className="text-gray-500">Status:</span>{' '}
                {sum.encounter.status}
              </div>
              <div>
                <span className="text-gray-500">Updated:</span>{' '}
                {new Date(sum.encounter.updatedAt).toLocaleString()}
              </div>
            </div>
          </section>
