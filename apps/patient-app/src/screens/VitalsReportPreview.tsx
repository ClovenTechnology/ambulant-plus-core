...
export default function VitalsReportPreview() {
  ...
  async function handleDownload() {
    ...
    link.href = pdfUrl;
    link.download = 'vitals_report.pdf';
    link.click();
  }
  ...
  async function handleShare() {
    ...
    const file = new File([blob], 'vitals_report.pdf', { type: 'application/pdf' });
  ...
  return (
    <div className="p-6 space-y-4">
      <h1 className="text-2xl font-bold">Vitals Report for Patient-123</h1>
      <p className="text-gray-500 text-sm">Generated: {new Date().toLocaleString()}</p>
      {loading && <p className="text-gray-500">Generating PDF…</p>}
      {pdfUrl && (
        <>
          <iframe src={pdfUrl} className="w-full h-[80vh] border rounded" title="Vitals Report" />
