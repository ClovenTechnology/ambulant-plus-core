...
// Format to 0–2 dp, returning an em dash for invalid
...
export function segmentColorFn(bounds: {
  warn: [number, number];
  alert: [number, number];
}) {
  return (ctx: ScriptableContext<'line'>) => {
    const v = ((ctx as any)?.p0?.parsed?.y ??
      (ctx as any)?.p1?.parsed?.y) as number | undefined;
    ...
    return undefined; // default colour
  };
}
...
// Draw thin vertical connectors that link BP sys/dia pairs
export const bpConnectorPlugin: Plugin<'line'> = {
  ...
  afterDatasetsDraw(chart: Chart) {
    ...
    const sysMeta = metas.find((m: any) =>
      m.dataset?.label?.includes('Systolic'),
    );
    const diaMeta = metas.find((m: any) =>
      m.dataset?.label?.includes('Diastolic'),
    );
    ...
    for (let i = 0; i < len; i++) {
      const sx = sysMeta.data[i]?.x;
      const sy = sysMeta.data[i]?.y;
...
// Centre text for Doughnut, e.g. “98% / SpO₂”
