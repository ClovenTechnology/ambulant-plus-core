...
export default function Countdown({ startsAt, endsAt, skewMs = 0 }: { startsAt: string; endsAt: string; skewMs?: number }) {
  ...
  useEffect(() => {
    const tick = () => { setNow(Date.now() + skewMs); rafRef.current = requestAnimationFrame(tick); };
    rafRef.current = requestAnimationFrame(tick);
    return () => { if (rafRef.current) cancelAnimationFrame(rafRef.current); };
  }, [skewMs]);

  const pct = clamp(((now - s) / total) * 100, -100, 300); // allow negative/overrun ranges
  const before = now < s;
    ...
    <div>
      ...
      <div className="mt-2 flex items-center justify-between">
        ...
        <div className="text-xs text-gray-600">
          ...
          {' â€“ '}
          {new Date(endsAt).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', hour12: false })}
        </div>
      </div>
    </div>
  );
