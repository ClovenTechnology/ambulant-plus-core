// apps/patient-app/app/api/v1/patients/[id]/otoscope-captures/route.ts
import { NextRequest, NextResponse } from 'next/server';
import fs from 'node:fs';
import fsp from 'node:fs/promises';
import path from 'node:path';
import crypto from 'node:crypto';

export const runtime = 'nodejs';

type Kind = 'photo' | 'video';

type Row = {
  id: string;
  patientId: string;
  kind: Kind;
  createdAt: string;

  bytes: number;
  sha256: string;

  mime: string;
  fileName: string; // stored on disk (tmp)
  fileUrl: string; // download url from this route

  // Best-effort media hints (optional)
  width?: number;
  height?: number;

  meta?: any;
};

const g = globalThis as any;
g.__OTOSCOPE__ = g.__OTOSCOPE__ || [];
const db: Row[] = g.__OTOSCOPE__;

function noStoreJson(body: any, init?: { status?: number }) {
  return NextResponse.json(body, {
    status: init?.status ?? 200,
    headers: {
      'Cache-Control': 'no-store, max-age=0',
      Pragma: 'no-cache',
    },
  });
}

function ensureTmp() {
  const dir = path.join(process.cwd(), '.tmp_uploads');
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  return dir;
}

function safeToken(s: string) {
  return String(s || '').replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 80);
}

async function parseMeta(metaPart: FormDataEntryValue | null) {
  if (!metaPart) return {};
  try {
    if (typeof metaPart === 'string') return JSON.parse(metaPart);
    const blob = metaPart as Blob;
    const text = await blob.text();
    if (!text) return {};
    return JSON.parse(text);
  } catch {
    return {};
  }
}

function sha256Hex(buf: Buffer) {
  return crypto.createHash('sha256').update(buf).digest('hex');
}

function streamFile(fullPath: string, mime: string, downloadName?: string) {
  const stat = fs.statSync(fullPath);
  const headers: Record<string, string> = {
    'Content-Type': mime || 'application/octet-stream',
    'Content-Length': String(stat.size),
    'Cache-Control': 'no-store, max-age=0',
    Pragma: 'no-cache',
  };

  if (downloadName) {
    headers['Content-Disposition'] = `inline; filename="${downloadName.replace(/"/g, '')}"`;
  }

  const nodeStream = fs.createReadStream(fullPath);
  return new NextResponse(nodeStream as any, { status: 200, headers });
}

/**
 * Best-effort image dimension sniff for PNG/JPEG.
 * Returns null if unknown.
 */
function sniffImageDims(buf: Buffer): { width?: number; height?: number } | null {
  // PNG: signature + IHDR
  if (buf.length >= 24) {
    const isPng =
      buf[0] === 0x89 &&
      buf[1] === 0x50 &&
      buf[2] === 0x4e &&
      buf[3] === 0x47 &&
      buf[4] === 0x0d &&
      buf[5] === 0x0a &&
      buf[6] === 0x1a &&
      buf[7] === 0x0a;

    if (isPng) {
      // IHDR starts at byte 12: length(4) + type(4) + data(13)
      const type = buf.toString('ascii', 12 + 4, 12 + 8);
      if (type === 'IHDR' && buf.length >= 24 + 8) {
        const w = buf.readUInt32BE(16);
        const h = buf.readUInt32BE(20);
        if (w > 0 && h > 0) return { width: w, height: h };
      }
    }
  }

  // JPEG: scan markers for SOF0/SOF2 etc
  if (buf.length >= 4 && buf[0] === 0xff && buf[1] === 0xd8) {
    let off = 2;
    while (off + 4 < buf.length) {
      if (buf[off] !== 0xff) {
        off += 1;
        continue;
      }
      const marker = buf[off + 1];
      off += 2;

      // Skip fill bytes
      while (marker === 0xff && off < buf.length) off += 1;

      // Markers without length
      if (marker === 0xd9 || marker === 0xda) break; // EOI or SOS

      if (off + 2 > buf.length) break;
      const len = buf.readUInt16BE(off);
      if (len < 2 || off + len > buf.length) break;

      // SOF markers that contain dims
      const isSof =
        (marker >= 0xc0 && marker <= 0xc3) ||
        (marker >= 0xc5 && marker <= 0xc7) ||
        (marker >= 0xc9 && marker <= 0xcb) ||
        (marker >= 0xcd && marker <= 0xcf);

      if (isSof && off + 7 < buf.length) {
        // layout: [len(2)] [precision(1)] [height(2)] [width(2)] ...
        const h = buf.readUInt16BE(off + 3);
        const w = buf.readUInt16BE(off + 5);
        if (w > 0 && h > 0) return { width: w, height: h };
      }

      off += len;
    }
  }

  return null;
}

const MAX_BYTES = 50 * 1024 * 1024; // dev guardrail (50MB)

/**
 * GET:
 * - ?download=1&id=<captureId> -> streams the file
 * - otherwise list captures with paging:
 *   ?limit=50&before=<ISO>&kind=photo|video
 */
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  const patientId = params.id;
  const url = req.nextUrl;

  if (url.searchParams.get('download') === '1') {
    const id = url.searchParams.get('id') || '';
    const row = db.find((r) => r.patientId === patientId && r.id === id);
    if (!row) return noStoreJson({ error: 'not_found' }, { status: 404 });

    const dir = ensureTmp();
    const fullPath = path.join(dir, row.fileName);
    if (!fs.existsSync(fullPath)) return noStoreJson({ error: 'file_missing' }, { status: 410 });

    return streamFile(fullPath, row.mime, row.fileName);
  }

  const limitRaw = Number(url.searchParams.get('limit') || '50');
  const limit = Math.min(Math.max(1, isFinite(limitRaw) ? limitRaw : 50), 200);
  const before = url.searchParams.get('before'); // ISO
  const kind = (url.searchParams.get('kind') || '') as Kind | '';

  const items = db
    .filter((r) => r.patientId === patientId)
    .filter((r) => (kind ? r.kind === kind : true))
    .filter((r) => {
      if (!before) return true;
      return r.createdAt < before;
    })
    .slice(-1500) // dev guard
    .sort((a, b) => (a.createdAt < b.createdAt ? 1 : -1))
    .slice(0, limit);

  const nextBefore = items.length ? items[items.length - 1].createdAt : null;
  return noStoreJson({ items, nextBefore });
}

/**
 * DELETE: ?id=<captureId>
 */
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  const patientId = params.id;
  const id = req.nextUrl.searchParams.get('id') || '';
  if (!id) return noStoreJson({ error: 'id_required' }, { status: 400 });

  const idx = db.findIndex((r) => r.patientId === patientId && r.id === id);
  if (idx === -1) return noStoreJson({ error: 'not_found' }, { status: 404 });

  const [row] = db.splice(idx, 1);

  try {
    const dir = ensureTmp();
    const fullPath = path.join(dir, row.fileName);
    await fsp.unlink(fullPath);
  } catch {
    // ignore
  }

  return noStoreJson({ ok: true });
}

/**
 * POST multipart/form-data:
 * - file: File (required)
 * - kind: "photo" | "video" (optional; inferred from mime if omitted)
 * - meta: JSON (optional)
 */
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  const patientId = params.id;

  const form = await req.formData();
  const file = form.get('file') as File | null;
  if (!file) return noStoreJson({ error: 'file_required' }, { status: 400 });

  if (typeof file.size === 'number' && file.size > MAX_BYTES) {
    return noStoreJson({ error: 'file_too_large', maxBytes: MAX_BYTES }, { status: 413 });
  }

  const meta = await parseMeta(form.get('meta'));

  const kindRaw = String(form.get('kind') || '').toLowerCase();
  const inferredKind: Kind =
    kindRaw === 'photo' || kindRaw === 'video'
      ? (kindRaw as Kind)
      : (file.type || '').startsWith('image/')
        ? 'photo'
        : 'video';

  const arrayBuf = await file.arrayBuffer();
  const buf = Buffer.from(arrayBuf);

  if (buf.byteLength > MAX_BYTES) {
    return noStoreJson({ error: 'file_too_large', maxBytes: MAX_BYTES }, { status: 413 });
  }

  const dir = ensureTmp();

  const id = crypto.randomUUID();
  const safePid = safeToken(patientId);

  const extRaw = (file.name?.split('.').pop() || 'bin').toLowerCase();
  const ext = extRaw.replace(/[^a-z0-9]/g, '').slice(0, 10) || 'bin';

  const prefix = inferredKind === 'photo' ? 'otos_photo' : 'otos_video';
  const fileName = `${prefix}_${safePid}_${id}.${ext}`;
  const fullPath = path.join(dir, fileName);

  await fsp.writeFile(fullPath, buf);

  const hash = sha256Hex(buf);

  const dims = inferredKind === 'photo' ? sniffImageDims(buf) : null;

  const fileUrl = `/api/v1/patients/${encodeURIComponent(
    patientId,
  )}/otoscope-captures?download=1&id=${encodeURIComponent(id)}`;

  const row: Row = {
    id,
    patientId,
    kind: inferredKind,
    createdAt: new Date().toISOString(),
    bytes: buf.byteLength,
    sha256: hash,
    mime:
      file.type ||
      (inferredKind === 'photo'
        ? ext === 'png'
          ? 'image/png'
          : ext === 'jpg' || ext === 'jpeg'
            ? 'image/jpeg'
            : 'application/octet-stream'
        : ext === 'mp4'
          ? 'video/mp4'
          : 'application/octet-stream'),
    fileName,
    fileUrl,
    meta,
    width: dims?.width,
    height: dims?.height,
  };

  db.push(row);

  return noStoreJson({ ok: true, item: row });
}
