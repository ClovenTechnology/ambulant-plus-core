...
export default function OtoscopePanel({ roomId }: { roomId?: string }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2">
        <a className={`px-2 py-1 border rounded text-xs ${sel ? '' : 'pointer-events-none opacity-50'}`}
           href={sel?.url ?? '#'} download={`otoscope_${sel?.ts ?? Date.now()}.jpg`}>
          Download
        </a>
      </div>
    </div>
