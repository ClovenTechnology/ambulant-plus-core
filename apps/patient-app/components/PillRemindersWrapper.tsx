...
export default function PillRemindersWrapper({
  pills: initialPills,
}: {
  pills?: { id?: string; name: string; dose?: string; time?: string; status?: string }[];
}) {
  ...
  async function load() {
    setLoading(true);
    try {
      // Only medication reminders for *today* (Pending), via backend filter
      ...
      if (res.ok && data.ok) {
        ...
        setReminders((prev) =>
          prev
            ? prev.map((r) =>
                r.id === id
                  ? { ...r, status: 'Taken', snoozedUntil: null }
                  : r
              )
            : prev
        );
            ...
            ? prev.map((r) =>
                r.id === id
                  ? {
                      ...r,
                      snoozedUntil:
                        data?.results?.[id]?.reminder?.snoozedUntil ??
                        r.snoozedUntil,
                    }
                  : r
              )
    ...
    <div className="space-y-2">
      ...
      <ul className="space-y-2 text-sm">
        {reminders?.map((r) => (
          <li
            key={r.id}
            className="border rounded p-2 bg-white flex items-start justify-between"
          >
            <div>
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              <div className="flex gap-1">
                <button
                  onClick={() => confirmTaken(r.id)}
                  className="px-2 py-1 text-xs rounded border"
                >
                  Confirm
                </button>
                <button
                  onClick={() => snooze(r.id, 15)}
                  className="px-2 py-1 text-xs rounded border"
                >
                  Snooze 15m
                </button>
