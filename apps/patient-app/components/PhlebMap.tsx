// apps/patient-app/components/PhlebMap.tsx
'use client';
...
export interface PhlebMapProps {
  open: boolean;
  onClose: () => void;
  coords: Coord[];
  phleb?: PhlebProfile;
  patientLocation?: CollectionLocation;
  labLocation?: CollectionLocation;
}
...
function MapFollower({
  ...
  useEffect(() => {
    if (target && map) {
      try {
        map.panTo([target.lat, target.lng], { animate: true } as any);
      } catch {
        // ignore
      }
    }
...
export default function PhlebMap({
  open,
  onClose,
  coords,
  phleb,
  ...
  // Reset trail + marker whenever coords change
  useEffect(() => {
  ...
  // Animate marker along polyline when map is open
  useEffect(() => {
    ...
    let start = coords[idx];
    let end = coords[Math.min(idx + 1, coords.length - 1)];
    let prog = 0;
  ...
  // Custom icons – pure CSS, no image assets needed
  ...
  // Don’t even mount Leaflet tree until we want the map
  if (!open) return null;
