...
public class BleBridge: CAPPlugin, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    private var maxReconnectAttempts = 6          // configurable via configure()
    ...
    // configure({ autoReconnect?: Bool, maxAttempts?: Int })
    @objc func configure(_ call: CAPPluginCall) {
        if call.get("autoReconnect") != nil {
            self.autoReconnect = call.getBool("autoReconnect", true)
        }
        if let m = call.getInt("maxAttempts") {
            self.maxReconnectAttempts = max(0, m)
        }
        call.resolve()
    }
    ...
    @objc func write(_ call: CAPPluginCall) {
        guard let charKey = call.getString("charKey") else {
            call.reject("Missing charKey"); return
        }
        guard let base64 = call.getString("base64"), let data = Data(base64Encoded: base64) else {
            call.reject("Missing/invalid base64"); return
        }
        guard let ch = charForKey[charKey], let p = peripheral else {
            call.reject("Characteristic not available or not connected"); return
        }
    ...
    public func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        for ch in chars {
            if let key = keyForChar[ch.uuid] {
                charForKey[key] = ch
            } else if let hit = uuidForKey.first(where: { $0.value == ch.uuid }) {
        ...
        if !uuidForKey.isEmpty {
            ...
            if allKnown || !charForKey.isEmpty {
