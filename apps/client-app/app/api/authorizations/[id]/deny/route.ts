  ...
  try {
    ...
    if (!sessionUserId(session)) {
      return NextResponse.json(
        { ok: false, error: "Please log in again before denying authorizations." },
        { status: 401 }
      );
    }
    ...
    if (!body?.decisionReason || typeof body.decisionReason !== "string") {
      return NextResponse.json(
        { ok: false, error: "Decision reason is required to deny an authorization." },
        { status: 400 }
      );
    }

    const res = await fetch(
      `${apigwBase()}/api/authorizations/${params.id}/deny`,
      {
        method: "POST",
    ...
    if (!res.ok || json?.ok === false) {
      return NextResponse.json(
        {
          ok: false,
          error: friendlyError(json?.error || "Failed to deny authorization."),
        },
