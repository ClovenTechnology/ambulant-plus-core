...
export default async function CarePortPage() {
  const items = await getBillableEvents();

  const pharmacyItems = items.filter(
    (x: any) => String(x.serviceType || "").toUpperCase() === "PHARMACY_ITEM"
  );

  const riderItems = items.filter(
    (x: any) => String(x.serviceType || "").toUpperCase() === "RIDER_DELIVERY"
  );
  ...
  return (
    <main style={{ padding: 32, maxWidth: 1480 }}>
      <div style={{ marginBottom: 24 }}>
        <div style={eyebrow}>CarePort Operations</div>
        <h1 style={{ margin: "8px 0 8px", fontSize: 34 }}>CarePort</h1>
        <p style={{ opacity: 0.82, margin: 0 }}>
          Sponsor-paid pharmacy items, chronic medicine fulfilment, eRx dispensing,
          delivery, patient co-pay, and settlement visibility for payer-funded care.
        </p>
      </div>

      <section style={metricGrid}>
        <Metric label="CarePort events" value={items.length} sub={`${pharmacyItems.length} pharmacy · ${riderItems.length} delivery`} />
        <Metric label="Gross value" value={money(grossMinor)} sub="Pharmacy + delivery events" />
      ...
      <section style={{ ...card, marginTop: 20 }}>
        <h2 style={{ marginTop: 0 }}>CarePort payer posture</h2>
        <div style={{ opacity: 0.82, lineHeight: 1.7 }}>
          CarePort links eRx fulfilment, pharmacy dispensing, chronic-medication cover,
          rider delivery, sponsor liability, patient co-pay, and settlement readiness.
          For Medical Aid operators, this is the lane where covered medicines and delivery
          services become auditable billable events.
        </div>
      </section>
                ...
                >
                  <div>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                      ...
                      <span style={mutedPill}>{item.providerLane || "PHARMACY"}</span>
                    ...
                    <div style={{ opacity: 0.72, fontSize: 13, marginTop: 8 }}>
                      Order: {item.carePortOrderId || metadata.orderId || "—"} · Claim:{" "}
                    ...
                    <div style={{ display: "grid", gap: 8, marginTop: 12 }}>
                      ...
                      <InfoLine label="Fulfilment" value={metadata.fulfillment || item.fulfillment || "Pharmacy / delivery"} />
                    </div>
