...
export async function GET(req: NextRequest, { params }: { params: { orderId: string } }) {
  ...
  try {
    const res = await fetch(`${apigwBase()}/api/careport/riders/me/jobs/${encodeURIComponent(orderId)}`, {
      method: 'GET',
    ...
    return NextResponse.json({ ok: false, error: error?.message || 'careport_rider_job_proxy_failed' }, { status: 502 });
  }
}
