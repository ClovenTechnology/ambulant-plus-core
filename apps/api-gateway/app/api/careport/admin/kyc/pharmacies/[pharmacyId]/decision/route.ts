...
export async function POST(req: NextRequest, { params }: { params: { pharmacyId: string } }) {
  ...
  try {
    ...
    if (!pharmacyId) {
      return json({ ok: false, error: "pharmacyId_required" }, 400);
    }
    ...
    if (!pharmacy) {
      return json({ ok: false, error: "pharmacy_not_found" }, 404);
    }
    ...
    return json(
      { ok: false, error: error?.message || "careport_admin_pharmacy_decision_failed" },
      error?.status || 500,
    );
  }
}
