...
import { beginCarePortRxPayment, serializeCarePortRxReservation } from "@/src/lib/careport-rx-commerce";
...
async function partnerOriginalPOST(req: NextRequest, { params }: { params: { sessionId: string } }) {
  ...
  try {
    ...
    const result = await beginCarePortRxPayment({
      who,
      orgId,
      correlationId,
    ...
    return NextResponse.json(
      { ok: false, error: error?.message || "rx_payment_init_failed", details: error?.details ?? null, correlationId },
