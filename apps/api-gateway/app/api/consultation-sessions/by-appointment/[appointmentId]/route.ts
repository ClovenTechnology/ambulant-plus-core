// apps/api-gateway/app/api/consultation-sessions/by-appointment/[appointmentId]/route.ts
...
export async function GET(
  ...
  try {
    const appointmentId = String(ctx.params.appointmentId || '').trim();
    const session = await getOrCreateSessionByAppointment(appointmentId);
    ...
    console.error('[consultation-sessions/by-appointment] failed', err);
