// apps/api-gateway/app/api/consultation-sessions/[id]/check-in/route.ts
...
export async function POST(
  ...
  try {
    const session = await checkInSession(String(ctx.params.id || '').trim());
    ...
    console.error('[consultation-sessions/check-in] failed', err);
    return json({ ok: false, error: err?.message || 'session_check_in_failed' }, 500);
  }
}
