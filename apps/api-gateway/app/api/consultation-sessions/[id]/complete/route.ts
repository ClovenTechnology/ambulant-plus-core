// apps/api-gateway/app/api/consultation-sessions/[id]/complete/route.ts
...
export async function POST(
  ...
  try {
    ...
    const session = await completeSession(String(ctx.params.id || '').trim(), payload);
    ...
    console.error('[consultation-sessions/complete] failed', err);
    return json({ ok: false, error: err?.message || 'session_complete_failed' }, 500);
  }
}
