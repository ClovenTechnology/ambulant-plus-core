// apps/api-gateway/app/api/consultation-sessions/[id]/start/route.ts
...
export async function POST(
  ...
  try {
    ...
    const session = await startSession(String(ctx.params.id || '').trim(), payload);
    ...
    console.error('[consultation-sessions/start] failed', err);
    return json({ ok: false, error: err?.message || 'session_start_failed' }, 500);
  }
}
