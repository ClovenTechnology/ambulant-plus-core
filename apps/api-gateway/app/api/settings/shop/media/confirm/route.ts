...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!enterpriseMediaObjectBelongsTo({ objectKey, kind, ownerId: targetId })) {
      return json({ ok: false, error: 'shop_media_object_invalid' }, 400);
    }
    ...
    if (targetKind === 'product') {
      const product = await prisma.shopProduct.findUnique({
        where: { id: targetId },
        select: { id: true, images: true, fallbackImage: true },
      });
      if (!product) return json({ ok: false, error: 'shop_media_target_not_found' }, 404);
      const images = Array.from(new Set([...(product.images || []), managedRef]));
      await prisma.shopProduct.update({
        where: { id: targetId },
        data: {
          images,
          ...(product.fallbackImage ? {} : { fallbackImage: managedRef }),
        },
      });
    } else {
      ...
      if (!variant) return json({ ok: false, error: 'shop_media_target_not_found' }, 404);
      await prisma.shopVariant.update({
        where: { id: targetId },
        data: { imageUrl: managedRef },
      });
    }

    return json({ ok: true, targetKind, targetId, managedRef, objectKey });
  } catch (error) {
    const media = enterpriseMediaErrorResponse(error);
    if (media) return json(media.body, media.status);
    console.error('[shop media] confirm failed', error);
    return json({ ok: false, error: 'shop_media_confirm_failed' }, 500);
  }
}
