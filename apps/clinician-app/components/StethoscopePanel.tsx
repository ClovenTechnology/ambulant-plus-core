...
export default function StethoscopePanel({ roomId, canSaveToSummary = false }: { roomId?: string; canSaveToSummary?: boolean }) {
  ...
  const ringRef = useRef<Float32Array>(new Float32Array(8000 * 3)); // 3s @8kHz
  ...
  const onAi = (obj: any) => {
    ...
    if (a.type === 'audio' && a.label) setAiNote(`${a.label} (conf ${Math.round((a.conf ?? 0) * 100)}%)`);
  };

  const url = useMemo(() => {
    const base = GW?.replace(/\/+$/, '') || '';
    const path = `/api/insight/stream?session=${encodeURIComponent(roomId || 'default')}`;
    return base ? `${base}${path}` : path; // works if clinician has a proxy route; else use full gateway base
  }, [roomId]);
  ...
  const connect = () => {
    ...
    es.addEventListener('ai', (e) => {
      try { onAi(JSON.parse((e as MessageEvent).data)); } catch {}
    });
  ...
  const saveToSummary = () => {
    // Gate behind premium; no-op if not allowed
    if (!canSaveToSummary) return;
    // TODO: POST blob to your attachments API with encounter/session metadata.
  };
  ...
  return (
    <div className="space-y-2">
      ...
      {aiNote && <div className="text-xs text-gray-700">AI: {aiNote}</div>}
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          className={`px-2 py-1 border rounded text-xs ${canSaveToSummary && wavUrl ? '' : 'opacity-50'}`}
          disabled={!canSaveToSummary || !wavUrl}
