...
export default function Sparkline({
  data, color = '#6366f1', height = 56, fill = true, showAxis = false,
}: {
  data: Pt[];
  color?: string;
  height?: number;
  ...
  if (!data || data.length === 0) {
    return <div className="h-[56px] grid place-items-center text-xs text-gray-400">no data</div>;
  }
      ...
      <ResponsiveContainer width="100%" height="100%">
        {fill ? (
          <AreaChart {...common}>
            ...
            />
            <Area dataKey="y" stroke={color} strokeWidth={2} fill={`url(#g-${id})`} isAnimationActive={false} />
          </AreaChart>
        ) : (
          <LineChart {...common}>
            {showAxis && <XAxis dataKey="t" hide />}
