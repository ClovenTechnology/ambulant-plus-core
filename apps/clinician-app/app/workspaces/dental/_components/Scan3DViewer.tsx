// apps/clinician-app/app/workspaces/dental/_components/Scan3DViewer.tsx
'use client';
...
export default function Scan3DViewer({
  ...
  const segmentationScheme: 'FDI' | 'universal' | null = evidence.meta
    ?.segmentation?.perTooth
    ? evidence.meta?.segmentation?.scheme ?? 'FDI'
    : null;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (
        ext === 'glb' ||
        ext === 'gltf' ||
        evidence.contentType?.includes('gltf')
      ) {
      ...
      root.traverse((obj: any) => {
        if (obj?.name) {
          objByName.set(String(obj.name), obj);
        }
      ...
      for (const m of meshList) {
        ...
        if (rootTooth?.name) {
          ensureBucket(String(rootTooth.name)).push(m);
        }
      }
      ...
      for (const m of meshList) {
        const keyUuid = String(m.uuid);
        const mat = m.material;
        const color = mat?.color ? mat.color.clone() : null;

        originalColors.set(keyUuid, color);
      }
      ...
      const onPointerDown = (ev: PointerEvent) => {
        ...
        const resolved = resolveToothNodeFromHit(hit);
        let overrideToothId: string | undefined;
        ...
        const meshId = String(
          pinTargetObj.name || hit.object.name || hit.object.uuid
        );
        ...
        let nLocal: [number, number, number] | undefined;
        ...
        const label = resolved?.meta
          ? formatBothSystems(resolved.meta).text
          : meshId
            ? `Mesh ${meshId}`
            : '3D Pin';
      ...
      const tick = () => {
        ...
        const selectedToothNodeName = targetNodeNameRef.current
          ? String(targetNodeNameRef.current)
          : '';
        const hoveredToothNodeName = hoverRef.toothNodeName
          ? String(hoverRef.toothNodeName)
          : '';

        for (const m of meshList) {
          const keyUuid = String(m.uuid);
          const orig = originalColors.get(keyUuid);

          if (m.material?.color && orig) {
            m.material.color.copy(orig);
          }
        }

        const applyToToothMeshes = (
          toothName: string,
          fn: (mesh: any) => void
        ) => {
        ...
        if (selectedToothNodeName) {
          applyToToothMeshes(selectedToothNodeName, (m) => {
            if (m.material?.color) {
              m.material.color.setHex(0x93c5fd);
            }
          });
        }

        if (
          hoveredToothNodeName &&
          hoveredToothNodeName !== selectedToothNodeName
        ) {
          applyToToothMeshes(hoveredToothNodeName, (m) => {
            if (m.material?.color) {
              m.material.color.setHex(0xbfdbfe);
            }
          });
        }
        ...
        for (const [toothName, meshes] of toothMeshesByRootName.entries()) {
          const show =
            toothName === selectedToothNodeName ||
            toothName === hoveredToothNodeName;
        ...
        for (const a of pinsRef.current) {
          ...
          const world = obj.localToWorld(
            new THREE.Vector3(pLocal[0], pLocal[1], pLocal[2])
          );
    ...
    return () => {
      ...
      try {
        st?.renderer?.domElement?.removeEventListener?.(
          'pointerdown',
          st?.onPointerDown
        );
        st?.renderer?.domElement?.removeEventListener?.(
      ...
      {!url ? (
        ...
        <>
          <div className="absolute left-3 top-3 z-10 rounded-full border bg-white/90 px-3 py-1 text-[11px] text-gray-700">
            Drag to orbit · Hover to identify tooth · Click to select + snap a
            3D pin
