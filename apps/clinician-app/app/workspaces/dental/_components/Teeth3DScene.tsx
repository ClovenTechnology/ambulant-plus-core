// apps/clinician-app/app/workspaces/dental/_components/Teeth3DScene.tsx
'use client';
    ...
    (async () => {
      ...
      const placeArc = (
        ...
        for (let i = 0; i < n; i += 1) {
          ...
          const idx = i + 1;
          const scale =
            idx <= 4 || idx >= 13 ? 1.15 : idx <= 6 || idx >= 11 ? 1.05 : 0.95;
      ...
      const onPointerDown = (ev: PointerEvent) => {
        ...
        const hits = raycaster.intersectObjects(
          Array.from(toothMeshes.values()),
          true
        );

        const hit = hits.find(
          (h) =>
            h.object?.userData?.toothId || h.object?.parent?.userData?.toothId
        );

        const toothId =
          hit?.object?.userData?.toothId ||
          hit?.object?.parent?.userData?.toothId;

        if (toothId) {
          onSelectTooth(String(toothId));
        }
      };
      ...
      const tick = () => {
        ...
        toothMeshes.forEach((mesh, id) => {
          ...
          if (!mesh.userData._baseY) {
            mesh.userData._baseY = mesh.position.y;
          }

          if (sel) {
            mesh.position.y = mesh.userData._baseY + 0.025;
          }
        });
