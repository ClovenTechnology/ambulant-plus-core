...
function toothNodeName(scheme: 'FDI' | 'universal', toothId: string) {
    ...
    try {
      await postAnnotation({
        ...
        payload: {
          ...
          x: 0.58,
          y: 0.42,
          label: `Adjust here (tooth ${universalToothIdFromLocation(selectedEvidence.location) ?? selectedEvidence.location.toothId})`,
                    ...
                    {revisions.map((r) => (
                      <li key={r.id} className="rounded-lg border bg-white p-2">
                        ...
                        <div className="mt-1 text-[11px] text-gray-600">
                          Tooth: {r.toothId ?? '—'} · Evidence: {r.evidenceIds.length} · Pins/comments: {r.annotationCount}
                        </div>
                      </li>
                    ))}
                  </ul>
          ...
          <section className="rounded-xl border bg-white shadow-sm">
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="rounded-lg border p-3 bg-gray-50">
                ...
                <div className="mt-1 text-sm text-gray-700">
                  Evidence posts with <span className="font-mono">meta.source: "live_capture"</span>. Your worker should PATCH{' '}
                  <span className="font-mono">/api/evidence</span> with final URL + status.
                </div>
              </div>
            </div>
          </section>
...
/* =========================================================
   3D Teeth Scene (premium: tooth-like lathe + enamel + rim light + gingiva silhouettes)
========================================================= */
function Teeth3DScene({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // tooth registry: id -> { root, meshes[] }
      const toothRegistry = new Map<string, { root: any; meshes: any[] }>();

      const makeEnamelMaterial = (baseHex: number) => {
        // MeshPhysicalMaterial gives a nicer “enamel” feel (clearcoat + specular)
        ...
        // Some builds of three support these extras; keep them best-effort
        try {
          (mat as any).ior = 1.45;
      ...
      const latheToothGeometry = (kind: 'incisor' | 'canine' | 'premolar' | 'molar') => {
        // Profile points (x=radius, y=height). Lathe around Y.
        // These are intentionally subtle; “tooth-like” without over-detail.
        const pts: any[] = [];
      ...
      const makeTooth = (id: string, kind: 'incisor' | 'canine' | 'premolar' | 'molar') => {
        ...
        // subtle “crown cap” highlight to give depth (tiny bevel feel)
        const cap = new THREE.Mesh(
          new THREE.SphereGeometry(0.036 + (kind === 'molar' ? 0.012 : kind === 'premolar' ? 0.008 : 0.006), 18, 18),
        ...
        // tiny “enamel rim” ring (adds premium glint under rim light)
        ...
        // slight “natural” tilt variance for realism (tiny)
      ...
      // Gingiva silhouettes (behind each row)
      ...
      const tick = () => {
        ...
        toothRegistry.forEach((entry, id) => {
          ...
          // enamel base: slightly warm white. highlights: blue for selected, green for “has findings”
...
function Scan3DViewer({
  ...
  useEffect(() => { targetNodeNameRef.current = targetNodeName; }, [targetNodeName]);
  useEffect(() => { disabledRef.current = !!disabled; }, [disabled]);
  useEffect(() => { onSelectToothRef.current = onSelectToothUniversal; }, [onSelectToothUniversal]);
  useEffect(() => { onAddModelPinRef.current = onAddModelPin; }, [onAddModelPin]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const onPointerMove = (ev: PointerEvent) => {
        ...
        const toothNodeName = String(resolved.node?.name || '');
        hoverRef.toothNodeName = toothNodeName;
    ...
    <div className="rounded-lg border bg-white p-3">
      ...
      {kind === 'scan_3d' ? (
        <div className="mt-2 flex flex-wrap items-center gap-3">
          ...
          <div className="text-[11px] text-gray-500">
            Click model to snap a true 3D pin (meshId + local hitpoint + normal).
          </div>
        </div>
      ) : null}
