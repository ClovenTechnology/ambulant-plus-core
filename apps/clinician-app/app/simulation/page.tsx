...
type Session = {
  ...
  leadClinicianId?: string | null; cohortRole?: string | null;
  clinicians?: Array<{ clinicianId: string; name?: string; specialty?: string | null; role?: string | null }>;
  ...
  async function enter(session: Session) {
    ...
    try {
      ...
      const query = new URLSearchParams({
        ...
        role: 'clinician',
        cohortRole: a.cohortRole || session.cohortRole || '',
        joinToken: a.token,
        jt: a.token,
        simulation: '1',
        simulationActor: 'clinician',
      });
          ...
          {(upcoming.length ? upcoming : sessions).map((session) => (
            <article key={session.appointmentId} className="rounded-3xl border bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm">
                <div><b>Your role:</b> {human(session.cohortRole || 'LEAD_CLINICIAN')}</div>
                <div className="mt-1"><b>Supervisor:</b> {session.supervisor?.name || 'Assigned'} · {session.supervisor?.mode || 'OBSERVE'}</div>
                {session.clinicians?.length ? <div className="mt-2"><b>Simulation cohort:</b> {session.clinicians.map((item) => `${item.name || 'Clinician'}${item.role ? ` (${human(item.role)})` : ''}`).join(', ')}</div> : null}
                {session.scenario ? <div className="mt-2"><b>Scenario:</b> {session.scenario}</div> : null}
                {session.expectedIoMTs?.length ? <div className="mt-2"><b>Expected IoMTs:</b> {session.expectedIoMTs.join(', ')}</div> : null}
              </div>
