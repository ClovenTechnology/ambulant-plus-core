...
function normalizeMember(raw: any): PracticeMember {
  return {
    ...
    fullName: raw.fullName ?? raw.name ?? raw.displayName ?? null,
    ...
    speciality: raw.speciality ?? raw.specialty ?? raw.discipline ?? null,
    city: raw.city ?? raw.practiceCity ?? raw.location?.city ?? null,
    country: raw.country ?? raw.practiceCountry ?? raw.location?.country ?? 'South Africa',
    lastSeenAt: raw.lastSeenAt ?? raw.last_active_at ?? null,
    createdAt: raw.createdAt ?? raw.created_at ?? raw.joinedAt ?? null,
  };
}
...
export default function PracticeMembersPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const js: MembersApiResponse = await res.json().catch(() => ({} as any));
  ...
  const counts = useMemo(() => {
    ...
    for (const m of members) {
      ...
      if (m.status === 'pending' || m.status === 'invited') pending++;
    }

    return { total: members.length, owners, clinicians, admins, billing, support, pending };
  }, [members]);
  ...
  const handleInviteSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const createdRaw = js?.member ?? js?.data ?? js ?? null;

      let created: PracticeMember | null = null;
      if (createdRaw) created = normalizeMember(createdRaw);
      else {
        created = {
      ...
      setInviteForm({ email: '', role: 'clinician', fullName: '' });
      setInviteOpen(false);
    } catch (err: any) {
      console.error('[practice/members] invite failed', err);
      toast(err?.message || 'Failed to send invite. Check /api/practice/members (POST).', 'error');
    } finally {
      setInviteBusy(false);
    }
  };
  ...
  const updateMemberRole = async (memberId: string, nextRole: PracticeMemberRole) => {
    ...
    try {
      ...
      const js = await res.json().catch(() => null);
      const updatedRaw = js?.member ?? js?.data ?? null;

      setMembers((prev) =>
        prev.map((m) =>
          m.id === memberId ? { ...m, role: updatedRaw ? normalizeMember(updatedRaw).role : nextRole } : m,
        ),
      );
    } catch (err: any) {
  ...
  const updateMemberStatus = async (memberId: string, nextStatus: PracticeMemberStatus) => {
    ...
    try {
      ...
      setMembers((prev) =>
        prev.map((m) =>
          m.id === memberId ? { ...m, status: updatedRaw ? normalizeMember(updatedRaw).status : nextStatus } : m,
        ),
      );
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Practice team</h1>
          <p className="mt-1 max-w-xl text-xs text-gray-600">
            Manage everyone linked to <span className="font-medium">{practiceName}</span>: owners, clinicians,
        ...
        <div className="flex flex-col items-end gap-2">
          <button
            ...
            className="rounded-full bg-black px-4 py-1.5 text-xs text-white hover:bg-gray-900"
          >
            + Invite member
          </button>
          <div className="text-right text-[11px] text-gray-500">
            Total: <span className="font-semibold">{counts.total}</span> · Pending:{' '}
      ...
      <section className="space-y-3 rounded border bg-white p-4 text-xs">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <input
              ...
              className="min-w-[200px] rounded border px-2 py-1 text-[11px]"
            />
          </div>
        ...
        <div className="grid gap-2 md:grid-cols-5">
          ...
          <div className="rounded border bg-white px-3 py-2">
            <div className="text-[11px] text-gray-500">Owners</div>
            <div className="text-lg font-semibold">{counts.owners}</div>
          </div>
          <div className="rounded border bg-white px-3 py-2">
            <div className="text-[11px] text-gray-500">Clinicians</div>
            <div className="text-lg font-semibold">{counts.clinicians}</div>
          </div>
          <div className="rounded border bg-white px-3 py-2">
            <div className="text-[11px] text-gray-500">Admin / Billing</div>
            <div className="text-lg font-semibold">{counts.admins + counts.billing}</div>
          </div>
          <div className="rounded border bg-white px-3 py-2">
            <div className="text-[11px] text-gray-500">Pending invites</div>
      ...
      <section className="grid gap-3 md:grid-cols-2">
        {filtered.map((m) => {
          ...
          const lastSeenStr = m.lastSeenAt ? new Date(m.lastSeenAt).toLocaleString() : 'Unknown';
          const createdStr = m.createdAt ? new Date(m.createdAt).toLocaleDateString() : null;

          return (
            <article key={m.id} className="flex flex-col gap-2 rounded border bg-white p-3 text-xs shadow-sm">
              <div className="flex items-start justify-between gap-2">
                <div className="space-y-0.5">
                  <div className="font-medium text-gray-900">{m.fullName ?? m.email ?? m.id}</div>
                  {m.email && <div className="text-[11px] text-gray-500">{m.email}</div>}
                  <div className="text-[11px] text-gray-500">
                    {m.speciality ? `${m.speciality} · ` : ''}
                    {[m.city, m.country].filter(Boolean).join(', ')}
                  </div>
                  {createdStr && <div className="text-[11px] text-gray-400">Joined {createdStr}</div>}
                </div>

                <div className="flex flex-col items-end gap-1">
                  <span className={'inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium ' + statusClass}>
                    {m.status === 'invited' ? 'Invited' : m.status === 'pending' ? 'Pending' : m.status === 'disabled' ? 'Disabled' : 'Active'}
                  </span>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 border-t pt-2">
                <div className="flex flex-wrap items-center gap-2">
                  <label className="inline-flex items-center gap-1 text-[11px] text-gray-600">
                    <span>Role:</span>
                    <select
                      disabled={!!roleUpdating[m.id] || roleIsOwner}
                      value={m.role}
                      onChange={(e) => updateMemberRole(m.id, e.target.value as PracticeMemberRole)}
                  ...
                  <label className="inline-flex items-center gap-1 text-[11px] text-gray-600">
                    ...
                    <select
                      ...
                      value={m.status}
                      onChange={(e) => updateMemberStatus(m.id, e.target.value as PracticeMemberStatus)}
                      className="rounded border px-2 py-0.5 text-[11px]"
                    >
                      <option value="active">Active</option>
                      <option value="pending">Pending</option>
                ...
                <div className="flex flex-col items-end gap-0.5 text-[10px] text-gray-500">
                  <span>
                    Last seen: <span className="font-mono">{lastSeenStr}</span>
                  </span>
                  {roleIsClinician && (
                    <Link href="/practice/profile" className="underline">
      ...
      {loading && <div className="text-xs text-gray-500">Loading members…</div>}

      {/* Invite modal */}
      {inviteOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="relative w-full max-w-md rounded-lg bg-white p-4 text-xs shadow-lg">
            <button
              type="button"
            ...
            <p className="mb-3 text-[11px] text-gray-500">
              Sends an email via your API gateway for the invitee to create or link an account.
            </p>

            <form onSubmit={handleInviteSubmit} className="space-y-3">
              <label className="flex flex-col gap-1">
                <span className="text-[11px] text-gray-700">Email address</span>
                <input
                  type="email"
                  required
              ...
              <label className="flex flex-col gap-1">
                <span className="text-[11px] text-gray-700">Full name (optional)</span>
                <input
                  type="text"
              ...
              <label className="flex flex-col gap-1">
                ...
                <select
                  ...
                  onChange={(e) => setInviteForm((prev) => ({ ...prev, role: e.target.value as PracticeMemberRole }))}
                  className="rounded border px-2 py-1 text-xs"
                >
                  <option value="clinician">Clinician</option>
                  <option value="admin">Admin</option>
                  <option value="billing">Billing</option>
                  <option value="support">Support</option>
                  <option value="owner">Owner (use sparingly)</option>
                </select>
              </label>
              ...
              <div className="flex justify-end gap-2 pt-1">
                <button
                  type="button"
                  onClick={() => setInviteOpen(false)}
                  className="rounded border bg-white px-3 py-1.5 text-[11px] hover:bg-gray-50"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={inviteBusy}
                  className="rounded bg-black px-3 py-1.5 text-[11px] text-white disabled:opacity-60"
                >
