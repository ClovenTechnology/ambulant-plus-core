...
export default function VaccinationsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const res = await fetch('/api/clinician/vaccinations', {
          cache: 'no-store',
        });
        ...
        if (!res.ok) {
          throw new Error(
            data?.error || data?.message || 'Failed to load vaccinations'
          );
        }
  ...
  async function onAdd(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (res.ok && res.json?.record) {
      setItems((prev) => [res.json.record, ...prev]);
    ...
    alert(res.json?.error || res.json?.message || 'Upload failed');
  }
  ...
  return (
    <main className="p-4 space-y-4">
      ...
      {loading ? (
        <div className="border rounded p-3 bg-white text-sm text-zinc-500">
              ...
              <div className="flex justify-between items-start gap-2">
                <div>
                  ...
                  <div className="text-xs text-zinc-400">
                    {v.date ? formatDate(v.date) : 'Unknown date'}{' '}
                  ...
                  {v.followupAt && (
                    <div className="text-xs text-blue-700 mt-1">
                      Next booster: {formatDate(v.followupAt)}
