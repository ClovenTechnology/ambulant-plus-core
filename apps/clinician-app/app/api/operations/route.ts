...
export async function GET() {
  try {
    const items = await prisma.operation.findMany({ orderBy: { createdAt: 'desc' }, take: 50 });
    return NextResponse.json({ ok: true, data: items }, { status: 200 });
  } catch (err) {
    console.error('operations.get.error', err);
    const mock = [{ id: 'OP-1', title: 'Appendicectomy', date: '2023-01-01', facility: 'Ambulant+ Surgical Centre', surgeon: 'Dr. Teeke', coClinicians: ['Dr. Naidoo'], clinicianCount: 2, recordedBy: 'Dr. Teeke' }];
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (files && files.file) {
      ...
      const key = `operations/${Date.now()}-${Math.random().toString(36).slice(2,8)}${ext}`;
    ...
    if (identity.role === 'clinician') {
      try {
        ...
        await prisma.operation.update({ where: { id: created.id }, data: { ehrTxId: tx.txId } });
      } catch (e) { console.warn('writeEhrIndex op failed', e); }
    }
