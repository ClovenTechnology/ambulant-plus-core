...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const diagnosedAt = getNullableString(form, 'diagnosedAt');
    const status = getString(form, 'status', 'Active');
    ...
    const onAmbulant = getBoolean(form, 'onAmbulant');
    ...
    if (uploadedFile instanceof File && uploadedFile.size > 0) {
      ...
      const key = `conditions/${Date.now()}-${Math.random()
        .toString(36)
        .slice(2, 8)}${ext}`;
    ...
    if (identity.role === 'clinician') {
      try {
        ...
        console.warn('writeEhrIndex failed', err);
      }
    }
