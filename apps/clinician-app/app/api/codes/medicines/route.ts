...
export async function GET(req: NextRequest) {
  try {
    ...
    const target = new URL('/api/codes/medicines', apiGatewayBase());
    ...
    return NextResponse.json(
      data ?? { ok: false, error: 'medicines_empty_response', items: [] },
      { status: upstream.status },
    );
  } catch (err: any) {
    return NextResponse.json(
      { ok: false, error: err?.message || 'medicines_proxy_failed', items: [] },
      { status: 502 },
    );
  }
}
