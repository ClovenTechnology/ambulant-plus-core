...
export async function GET() {
  try {
    const items = await prisma.vaccination.findMany({ orderBy: { createdAt: 'desc' }, take: 50 });
    return NextResponse.json({ ok: true, data: items }, { status: 200 });
  } catch (err) {
    console.error('vaccinations.get.error', err);
    const mock = [{ id: 'VAX-1', vaccine: 'COVID-19 (Pfizer)', date: '2022-11-01', batch: 'PF12345', clinician: 'Dr. Adeola', facility: 'Lagos General', recordedBy: 'Dr. Adeola' }];
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (files && files.file) {
      ...
      const key = `vaccinations/${Date.now()}-${Math.random().toString(36).slice(2,8)}${ext}`;
    ...
    if (identity.role === 'clinician') {
      try {
        ...
        await prisma.vaccination.update({ where: { id: created.id }, data: { ehrTxId: tx.txId } });
      } catch (e) { console.warn('writeEhrIndex vax failed', e); }
    }
