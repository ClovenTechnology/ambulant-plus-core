// apps/clinician-app/app/api/settings/refunds/route.ts
...
async function proxy(req: NextRequest, method: 'GET' | 'PUT') {
  ...
  const res = await fetch(`${GW}/api/settings/refunds`, init);
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error('[clinician-app] refund settings proxy failed', err);
...
export async function PUT(req: NextRequest) {
  try {
    ...
    console.error('[clinician-app] refund settings save proxy failed', err);
    return json({ ok: false, error: err?.message || 'gateway_failed' }, 500);
  }
}
