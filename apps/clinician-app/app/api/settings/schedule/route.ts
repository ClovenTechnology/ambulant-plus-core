...
async function proxy(req: NextRequest, method: 'GET' | 'PUT') {
  ...
  const res = await fetch(`${GW}/api/settings/schedule`, init);
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error('[clinician-app] schedule settings proxy failed', err);
...
export async function PUT(req: NextRequest) {
  try {
    ...
    console.error('[clinician-app] schedule settings save proxy failed', err);
    return json({ ok: false, error: err?.message || 'gateway_failed' }, 500);
  }
}
