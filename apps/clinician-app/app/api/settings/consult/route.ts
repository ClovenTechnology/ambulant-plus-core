...
async function proxy(req: NextRequest, method: 'GET' | 'PUT') {
  ...
  const res = await fetch(`${GW}/api/settings/consult`, init);
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error('[clinician-app] consult settings proxy failed', err);
...
export async function PUT(req: NextRequest) {
  try {
    ...
    console.error('[clinician-app] consult settings save proxy failed', err);
    return json({ ok: false, error: err?.message || 'gateway_failed' }, 500);
  }
}
