      ...
      <aside className="lg:sticky lg:top-6 lg:self-start">
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            {navItems.map((item) =>
              item.href ? (
                <Link
                  key={item.label}
                  href={item.href}
                  className="block rounded-2xl border border-slate-200 bg-white px-4 py-3 transition hover:border-sky-300 hover:bg-sky-50"
                >
          ...
          <div className={`mt-5 rounded-2xl border px-4 py-3 text-xs leading-5 ${accent}`}>
            Role-specific menu active. This workspace only exposes navigation relevant to this MedReach role.
          </div>
        </div>
      </aside>
