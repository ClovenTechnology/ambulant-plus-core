...
c.on("message", (topic, buf) => {
  ...
  if (tail === "ecg.stop") {
    if (ecgTimer) {
      ...
      ecgTimer = null;
      const dur = Date.now() - ecgStart;
      c.publish(`${ROOT}/${ID}/ecg/summary`, JSON.stringify({
        tsStart: ecgStart, tsEnd: Date.now(), hrAvg: 72, hrMax: 96, hrMin: 58, duration_ms: dur
      }));
    }
  }
});
