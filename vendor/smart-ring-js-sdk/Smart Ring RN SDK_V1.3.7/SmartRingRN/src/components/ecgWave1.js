...
const EcgWave = (prop) => {
  ...
  let drawnPoints = []; // 存储已绘制的点
  let animationId = null; // 动画帧ID
  let currentIndex = 0; // 当前绘制点索引
  const POINTS_PER_FRAME = 10; // 每帧绘制点数

  function requestECGWaveLayout() {
    if (ecgList.length === 0) return;
    ...
    const animate = () => {
      ...
      // 添加新点
      const endIndex = Math.min(currentIndex + POINTS_PER_FRAME, ecgList.length);
      ...
      if (currentIndex < ecgList.length) {
